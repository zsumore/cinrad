...
public class MQFilter {
	...
	// distance in km
	public MQFilter(double minDistance, double maxDistance, double minValue,
			double maxValue, double minAzimuth, double maxAzimuth,
			double minHeight, double maxHeight, Envelope extentFilter) {
	...
	public void setExtentFilter(Envelope extentFilter) {
		this.extentFilter = extentFilter;

		if (extentFilter != null) {

			extentGeometry = geoFactory.toGeometry(extentFilter);
	...
	public Envelope getExtentFilter() {
		return extentFilter;
	}
	...
	public boolean accept(double dvalue, double azimuth, double range,
			double height) {
		// Check if startrange is within min and max distance range
		if (range < minDistance || range > maxDistance) {
			return false;
	...
	public boolean accept(double dvalue) {
		// Check if value is within min and max value
		int len = (minValue.length < maxValue.length) ? minValue.length
				: maxValue.length;
	 ...
	 * 
	 * 
	 * try { poly = poly.intersection(rangeGeometry); //poly =
	 * poly.difference(minRangeGeometry); return poly; } catch (Exception e) {
	 * return poly; } } */

	/**
	 * For Level-III filtering, category values are compared to max and min
	 * value pairs supplied in filter. For non-numeric categories (such as
	 * 'RF'), the categoryOverrides is used to include that category. By
	 * default, non-numeric categories are ignored.
	 */
	...
	/**
	 * Takes category values and compares to max and min value pairs supplied in
	 * filter. For non-numeric categories (such as 'RF'), the categoryOverrides
	 * is used to include that category. By default, non-numeric categories are
	 * ignored.
	 */
	public void setValueIndices(String[] categoriesFromHeader) throws Exception {
		...
		for (int n = 0; n < categoriesFromHeader.length; n++) {
			...
			if (categoriesFromHeader[n] != null
					&& categoriesFromHeader[n].trim().length() > 0) {
				categoriesFromHeader[n] = categoriesFromHeader[n].replaceAll(
						">", "").replaceAll("<", "");

				// System.out.println("categories[" + n + "] = " +
				// categoriesFromHeader[n]); // DEBUG

				// ---- NON-NUMERIC CATEGORIES
				if (categoriesFromHeader[n].equalsIgnoreCase("RF")) {
					for (int i = 0; i < categoryOverrides.length; i++) {
						if (categoryOverrides[i].trim().toUpperCase()
								.equals("RF")) {
							printCat[n] = true;
				 ...
				 * ------------------------------------------------------------ */
					...
					for (int pair = 0; pair < len; pair++) {
						...
						// System.out.println("Value in cat " + n + " = " +
						// value); // DEBUG
	...
	public String toString() {
		return "WCT FILTER: \n\t" + printValueRanges() + "\n\tDIST["
				+ minDistance + " to " + maxDistance + "]\n\tAZIMUTH["
				+ minAzimuth + " to " + maxAzimuth + "]\n\tHEIGHT[" + minHeight
				+ " to " + maxHeight + "]\n\t EXTENT[" + extentFilter
				+ "]\n\tCATEGORY OVERRIDES: "
				+ Arrays.deepToString(categoryOverrides);
	}
