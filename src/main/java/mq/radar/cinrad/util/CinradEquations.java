...
import mq.radar.cinrad.MQUtils;

public class CinradEquations {
	...
	public static double getRelativeBeamHeight(double elevAngleInDegrees,
			double range) {
		return (getRelativeBeamHeight(
				Math.cos(Math.toRadians(elevAngleInDegrees)),
				Math.sin(Math.toRadians(elevAngleInDegrees)), range));
	}

	public static double getRelativeBeamHeight(double elevAngleRadiansCos,
			double elevAngleRadiansSin, double range) {
		...
		double height = slantRange
				* elevAngleRadiansSin
				+ (Math.pow(slantRange, 2) / (2 * refractionCoef * earthRadius))
				+ 0; // 50 is average height of radar above ground

		// sr*sin + sr^2/x = h
		// h/sr^2 = sin/sr + x

		// height += nexradHeader.getAlt(); // Add height of radar site -- DON'T
		// DO THIS FOR CURSOR - ATMS PEOPLE LIKE RELATIVE ELEV.
		// height *= 0.3048; // convert to meters from feet
		return height;
	}

	public static double getRelativeBeamHeight_old(double elevAngleCos,
			double elevAngleSin, double range) {

		double range_in_nmi = range * 0.000539957;
		// below returns relative elevation in feet
		double height = ((Math.pow(range_in_nmi, 2) * Math.pow(elevAngleCos, 2)) / 9168.66 + range_in_nmi
				* elevAngleSin) * 6076.115;
		// height += nexradHeader.getAlt(); // Add height of radar site -- DON'T
		// DO THIS FOR CURSOR - ATMS PEOPLE LIKE RELATIVE ELEV.
		height *= 0.3048; // convert to meters from feet
		return height;
	...
	public static double getRelativeBeamHeight2(double elevAngleCos,
			double elevAngleSin, double range) {
		// 1. convert surface range to slant range
		...
		double height = Math.sqrt(Math.pow(slantRange, 2)
				+ Math.pow(refractionCoef * earthRadius, 2) + 2 * slantRange
				* refractionCoef * earthRadius * elevAngleSin)
				- (refractionCoef * earthRadius) + 0; // 50 is average height of
														// radar above ground
	...
	public static String convertMilliseconds(int milliseconds) {
		...
		return (MQUtils.DECFMT_00.format(scan_hour)
				+ MQUtils.DECFMT_00.format(scan_minute) + MQUtils.DECFMT_00
				.format(scan_seconds));
