...
public class JNXNidsheader extends Nidsheader {
	private final Logger logger = LoggerFactory.getLogger(JNXNidsheader.class);
	boolean isZ;
	int encrypt;

	// STEVE ANSARI ADDITION
	public byte[] readZLibNIDS(ucar.unidata.io.RandomAccessFile raf)
			throws IOException {
		...
		// some product require to go through the whole file to build the struct
		// of file.
		...
		rc = raf.read(b);
		if (rc != readLen) {
			logger.info(" error reading nids product header");
		}
		...
		switch (type) {
		case 0:
			logger.info("ReadNexrInfo:: Unable to seek to ID ");
			break;
		case 1:
		case 2:
		case 3:
		...
		if (zlibed == 1) {
			...
			if (uncompdata == null) {
				logger.info("ReadNexrInfo:: error uncompressing image");
			}
		} else {
	...
	/* * Name: IsZlibed** Purpose: Check a two-byte sequence to see if it
	 * indicates the start of* a zlib-compressed buffer* */
