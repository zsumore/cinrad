...
/**
 * Characterises the compression status of a stream, and provides methods for
 * decompressing it.
 * 
...
public abstract class Compression {
	 ...
	 * 
	 * @param magic
	 *            a buffer containing the first {@link #MAGIC_SIZE} bytes of
	 *            input of the stream to be characterised
	 * @return a <tt>Compression</tt> object of the type represented by
	 *         <tt>magic</tt>
	 * @throws IllegalArgumentException
	 *             if <tt>magic.length&lt;MAGIC_SIZE</tt>
	 */
		...
		if (magic.length < MAGIC_SIZE) {
			throw new IllegalArgumentException(
					"Magic buffer must be at least MAGIC_SIZE=" + MAGIC_SIZE
							+ " bytes");
		} else if (magic[0] == (byte) 0x1f && magic[1] == (byte) 0x8b) {
			return GZIP;
		} else if (magic[0] == (byte) 'B' && magic[1] == (byte) 'Z'
				&& magic[2] == (byte) 'h') {
			return BZIP2;
		} else if (magic[0] == (byte) 0x1f && magic[1] == (byte) 0x9d) {
			return COMPRESS;
		} else {
			return NONE;
		}
	...
	/**
	 * A Compression object representing no compression (or perhaps an unknown
	 * one). The <tt>decompress</tt> method will return the raw input stream
	 * unchanged.
	 */
	...
	public static final Compression GZIP = new Compression("gzip") {
		public InputStream decompress(InputStream raw) throws IOException {
			 ...
			 * GZIPInputStream.markSupported() returns true; however instances
			 * of this class do not support marking, which screws up some things
			 * that the DataSource class tries to do. So we fiddle the inflating
			 * stream to tell the truth. */
			/* Note this seems to be not uncommon in decompression streams (had
			 * to fix the same bug in the UncompressInputStream implementation
			 * used here too). */
